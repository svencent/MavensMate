...
describe('mavensmate new-lightning-component-cli', function(){
  ...
  it('should accept a ui flag', function(done) {    
    ...
    cliClient.program._events['new-lightning-component']();
    
    executeCommandStub.calledOnce.should.equal(true);
    assert(executeCommandStub.calledWith('new-lightning-component', { args: { ui: true } }));
    cmd.ui = false;
    done();
  });
  ...
  it('should accept stdin', function(done) {
    ...
    getPayloadStub().then(function() {
      executeCommandStub.calledOnce.should.equal(true);
      assert(executeCommandStub.calledWith('new-lightning-component', { foo : 'bar' }));
      done();
    });
  });
});