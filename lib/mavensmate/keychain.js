 ...
 * @param  {String} type - either "accessToken" or "refreshToken"
 * @return {Nothing}
 */
...
KeychainService.prototype.replacePassword = function(id, password, type) {
  ...
  if (this.useSystemKeychain()) {
    ...
    throw new Error('Attempt to replace password failed: system keychain service not supported');
  }
};
...
KeychainService.prototype.getPassword = function(id, type) {
  ...
  if (this.useSystemKeychain()) {
    ...
    throw new Error('Attempt to retrieve password failed: system keychain service not supported');
  }
};
