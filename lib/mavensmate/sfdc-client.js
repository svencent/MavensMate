...
SalesforceClient.prototype.runTests = function(tests) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    logger.debug('submitting tests to be run via the bulk api:', tests);
    ...
    self.conn.bulk.load('ApexTestQueueItem', 'insert', tests, function(err, res) {
      if (err) {
        logger.error('Error loading ApexTestQueueItem', err);
        reject(err);
      } else {
        logger.info('bulk loaded ApexTestQueueItems', res);
        if (!res[0].id && _.isArray(res[0].errors) && res[0].errors.length > 0) {
          logger.error(res[0].errors);
          return reject(new Error(res[0].errors));
        }
        ...
        self.conn.tooling.query('Select ParentJobId FROM ApexTestQueueItem WHERE Id = \''+testQueueItemId+'\'', function(err, res) {
          if (err) {
            ...
            logger.debug('response from ApexTestQueueItem query', res);
