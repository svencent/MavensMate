...
function SalesforceClient(opts) {
  ...
  this.apiVersion = config.get('mm_api_version') || '33.0';
  ...
  return new Promise(function(resolve, reject) {
    if (!self.initialized) {
      ...
      if (accessToken) {
        ...
        self.conn = new jsforce.Connection({ sessionId: accessToken, instanceUrl: self.getInstanceUrl(), logLevel: config.get('mm_sfdc_api_log_level') || 'FATAL', version: self.apiVersion });
          ...
          if (err) {
            ...
            self._login()
              ...
              .catch(function(error) {
                logger.error('Could not log into salesforce with provided credentials: ');
                logger.error(error);
                reject(error);
              })
              .done();
            ...
            self.describe()
              ...
              .catch(function(error) {
                logger.error('Could initiate salesforce client: '+error);
                reject(error);
              })
              .done();
          }
...
SalesforceClient.prototype._login = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.conn = new jsforce.Connection({ logLevel: config.get('mm_sfdc_api_log_level') || 'FATAL', version: self.apiVersion, loginUrl: self._getLoginUrl() });
    self._initRefreshHandler();
...
SalesforceClient.prototype.createApexOrVisualforceFile = function(mavensMateFile) {
  logger.info('createApexOrVisualforceFile');
