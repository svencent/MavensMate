...
Client.prototype.addProjectById = function(projectId, sfdcClient) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (!projectPath) {
      return reject(new Error('Could not find project. Please ensure your mm_workspace global setting is configured properly and your project is located in a valid mm_workspace'));
    }
    ...
    project.initialize(false)
      ...
      .catch(function(err) {
        if (err.message.indexOf('INVALID_LOGIN') >= 0) {
    ...
    _.each(finishCommandsForCommand, function(c) {
      if (c === 'index-apex-class') {
        _.each(body.paths, function(p) {
          finishPromises.push(self.executeCommandForProject(project, 'index-apex-class', { className: p }, editor));
        });
      }
      if (c === 'index-apex') {
        finishPromises.push(self.executeCommandForProject(project, 'index-apex', null, editor));
      }
    });
 ...
 * @return {Nothing}
 */
Client.prototype.executeCommandForProject = function(projectOrProjectId, command, payload, editor) {
  var self = this;
  editor = editor || process.env.MAVENSMATE_EDITOR;
  return new Promise(function(resolve, reject) {
    ...
    if (_.isString(projectOrProjectId)) {
      ...
      commandInstance = new Client[commandClass](self, project, payload, editor);
    } else if (_.isObject(projectOrProjectId)) {
      commandInstance = new Client[commandClass](self, projectOrProjectId, payload, editor);
    } else {
      commandInstance = new Client[commandClass](self, self.getProjects()[0], payload, editor);
    }
 ...
 * @return {Nothing}
 */
Client.prototype.executeCommand = function(command, payload, editor) {
  var self = this;
  editor = editor || process.env.MAVENSMATE_EDITOR;
  return new Promise(function(resolve, reject) {
    logger.info('\n\n==================> executing command: '+command+'\n');
    logger.debug('executeCommand payload is: '+JSON.stringify(payload));
    var commandClass = up.capitalize(up.camelize(command))+'Command'; // => new-project -> NewProjectCommand
    if (!Client[commandClass]) {
      self._handleCommandResult({
        error: new Error('Command not supported: '+command),
    ...
    logger.debug('command class is: '+commandClass);
    var commandInstance = new Client[commandClass](self, self.getProjects()[0], payload, editor);
    commandInstance.execute()
      .then(function(result) {
        self._handleCommandResult({
          result: result,
