...
Client.prototype._initConfig = function() {
  ...
  if (this.editor.toLowerCase().indexOf('sublime') >= 0) {
    ...
    if (fs.existsSync(userClientSettingsPath)) {
      ...
      self._monitorConfigFile(userClientSettingsPath);
    }
    ...
    if (fs.existsSync(atomSettingsPath)) {
      ...
      if (atomSettings['*'] && atomSettings['*'] && atomSettings['*']['MavensMate-Atom']) {
        ...
        self._monitorConfigFile(atomSettingsPath); 
      }
    }  
  }   
 ...
 */
Client.prototype.runFinishCommands = function(project, command, body) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    commandInstance.execute()
      .then(function(result) {
        self.runFinishCommands(commandInstance.getProject(), command, payload);
        self.handleCommandResult({
          result: result,
          resolve: resolve,
          reject: reject
