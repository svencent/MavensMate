      ...
      self.sfdcClient.retrieveUnpackaged(childRetrievePackage)
        ...
        .then(function(retrievePath) {         
          ...
          finder.on('error', function (err) {
            reject(new Error('Could not crawl retrieved metadata: '+err.message));
          });
        })
        .catch(function(err) {
          reject(new Error('Could not index metadata type '+xmlName+': ' +err.message));
        })
        .done();
    } catch(e) {
      reject('Could not index metadata type '+xmlName+': ' +e.message);
    }
  });
};
...
IndexService.prototype.indexServerProperties = function(types) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    _.each(types, function(type) {
      typeMap[type] = self.metadataService.getTypeByXmlName(type);
    ...
    Promise.all(listRequests)
      ...
      .catch(function(error) {
        ...
        reject(new Error('Could not finish indexing server properties: '));
      })
      .done();
  });
};
