...
var RefreshService = function(project) {
 ...
 * @return {Promise} 
 */
RefreshService.prototype.refreshFromServer = function(metadata) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (lightningMetadata.length > 0) {
      ...
      var metadataService = new MetadataService({ sfdcClient: self.project.sfdcClient });
      metadataService.objectify(metadata)
        .then(function(metadataPayload) {
          logger.debug(metadataPayload);
          return self.project.sfdcClient.retrieveUnpackaged(metadataPayload);
        })
