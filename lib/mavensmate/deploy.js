...
Deploy.prototype.getNamedDeployments = function() {
  ...
  var namedDeployments = [ 'package.xml' ]; // this is the default deployment, based on the project's package.xml
  if (!fs.existsSync(path.join(this.project.path, 'deploy'))) {
    ...
    _.each(util.listDirectories(path.join(this.project.path, 'deploy')), function(savedDeployPath) {
      namedDeployments.push(path.basename(savedDeployPath));
    });
    return namedDeployments;
  }
};
...
Deploy.prototype.executeRemote = function(deployOptions) {
  ...
  return new Promise(function(resolve, reject) {
    if (!self._getTargets() || self._getTargets().length === 0) {
...
Deploy.prototype._deployToTarget = function(target, deployPath, deployOptions) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var getPasswordPromise = target.password ? new Promise(function(resolve) { resolve(target.password); }) : self.project.keychainService.getPassword(target.id);
