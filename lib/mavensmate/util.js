        ...
        .pipe(fstream.Writer(tmpZipLocation))
          ...
          .on('close', function() {            
            ...
            if (self.isMac() || self.isLinux()) {
              unzipCommand = spawn('unzip', [ tmpZipLocation, '-d', destination ]);
            } else if (self.isWindows()) {
              unzipCommand = spawn('cscript', [path.join(__dirname, '..', '..', 'bin', 'unzip.vbs'), tmpZipLocation, destination ]);
            }

            unzipCommand.stderr.on('data', function (data) {
              logger.error('ERR unzipping:');
              logger.error(data);
              reject(new Error('Could not extract and write stream to file system.'));
            });
