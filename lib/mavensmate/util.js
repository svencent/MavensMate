...
MavensMateUtil.prototype.zipDirectory = function(directoryToZip, zipFileDestination, zipFileName) {
  var self = this;
  return new Promise(function(resolve, reject) {
    ...
    zipFileName = zipFileName || 'unpackaged.zip';
    var newZipFilePath = path.join(zipFileDestination, zipFileName);

    logger.debug('zipping directory --> '+directoryToZip);
    logger.debug(newZipFilePath);

    var zipCommand;
    if (self.isMac() || self.isLinux()) {
      zipCommand = spawn('zip', [ '-r', newZipFilePath, directoryToZip, '-q' ], {
        cwd: directoryToZip,
        stdio: [ 'ignore', 'ignore', 'pipe' ]
      });
    } else if (self.isWindows()) {
      zipCommand = spawn('cscript', [path.join(__dirname, '..', '..', 'bin', 'zip.vbs'), newZipFilePath, '' ], {
        cwd: directoryToZip,
        stdio: [ 'ignore', 'ignore', 'pipe' ]
      });
    }

    zipCommand.stderr.on('data', function (data) {
      logger.error('ERR zipping:');
      logger.error(data);
      return reject(new Error('Could not zip files.'));
    });

    zipCommand.on('close', function (code) {
      if (code !== 0) {
        return reject(new Error('Could not zip files'));
      } else {
        return resolve();                
