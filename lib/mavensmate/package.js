...
PackageService.prototype.insert = function(metadata, writeToDisk) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.deserialize()
      ...
      .catch(function(err) {
        reject(new Error('Could not insert metadata to package: '+err.message));
      })
      .done();
  });
};
...
PackageService.prototype.deserialize = function() {
  ...
  return new Promise(function(resolve, reject) {
    var pkg = {};
    logger.debug('deserializing: '+self.location);
    if (!self.location) {
      reject(new Error('Please set package.xml location'));
    } else {
