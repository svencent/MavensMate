...
ResourceBundleService.prototype.create = function(files) {
  ...
  return new Promise(function(resolve, reject) { 
    ...
    _.each(files, function(f) {
      ...
      if (m.type.xmlName !== 'StaticResource') {
        throw new Error('File is not a static resource');
      } else {
        metadata.push(m);
        var bundlePath = path.join(self.project.path, 'resource-bundles', [m.name, 'resource'].join('.'));
        fs.ensureDirSync(bundlePath);
        writePromises.push(self._write(m.path, bundlePath));
      }
    });
