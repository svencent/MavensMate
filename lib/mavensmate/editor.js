  ...
  if (this.client.editor.toLowerCase() === 'atom') {
    ...
    try {
      ...
      if (util.isWindows()) {
        ...
        var sublApp = config.get('mm_osx_plugin_client_app_name');
        var sublAppPath = path.join(process.env.HOME, '/Applications/', sublApp, '/Contents/SharedSupport/bin/subl');
        if (fs.existsSync(sublAppPath)) {
          sublConfig = sublAppPath;
        } else {
          sublConfig = _path.join('/Applications/', sublApp, '/Contents/SharedSupport/bin/subl');
        }
      } else { 
