...
EditorService.prototype.open = function(path) {
  var self = this;
  return new Promise(function(resolve, reject) {
    if (self.client.editor.toLowerCase() === 'atom') {
    ...
    } else if (self.client.editor.toLowerCase() === 'sublime') {
      var sublLocationConfig, pathSublLocation;
      sublLocationConfig = config.get('mm_subl_location')[util.platformConfigKey];
      ...
      if (sublPath) {
        ...
        reject(new Error('Invalid sublime text executable. Ensure mm_subl_location is configured properly.'));
      }
    }
  });
};
