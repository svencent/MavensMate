...
EditorService.prototype.launchUI = function(commandName) {
  ...
  if (os.platform() === 'darwin') {
    var windowServerPath = util.escapeShell([path.join(util.getAppRoot(), 'bin', 'MavensMateWindowServer.app')]);    
    var windowServerChildProcess = spawn('open', ['-n', windowServerPath, '--args', '-url', 'http://localhost:'+portNumber+'/app/'+self._uriMap[commandName]], {
      detached: true
    });
...
EditorService.prototype.open = function(path) {
  if (this.client.editor.toLowerCase() === 'atom') {
    try {
      if (which.sync('atom')) {
        exec(util.escapeShell([which.sync('atom'), path]), function (error, stdout, stderr) {
      ...
      if (atomExecConfig) {
        try {
          exec(util.escapeShell([atomExecConfig, path]), function (error, stdout, stderr) {
            logger.debug('Result of path open in atom: ');
            logger.debug(error);
            logger.debug(stdout);
            logger.debug(stderr);
    ...
    try {
      if (which.sync('subl')) {
        exec(util.escapeShell([which.sync('subl'), path]), function (error, stdout, stderr) {
      ...
      // logger.debug(util.escapeShell([sublConfig, path]));
      if (sublConfig) {
        try {
          exec(util.escapeShell([sublConfig, path]), function (error, stdout, stderr) {
            logger.debug('Result of path open in sublime text: ');
            logger.debug(error);
            logger.debug(stdout);
            logger.debug(stderr);
          });
