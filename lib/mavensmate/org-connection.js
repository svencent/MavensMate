 ...
 */
OrgConnectionService.prototype.add = function(username, accessToken, refreshToken, orgType, loginUrl) {
  var self = this;
  return new Promise(function(resolve, reject) {
    ...
    if (loginUrl) {
      newConnection.loginUrl = loginUrl;
    }

    if (self.keychainService.useSystemKeychain()) {
      self.keychainService.storePassword(connectionId, password, 'accessToken');
      self.keychainService.storePassword(connectionId, password, 'refreshToken');
    } else {
 ...
 * @return {Promise}
 */
OrgConnectionService.prototype.update = function(id, username, accessToken, refreshToken, orgType, loginUrl) {
  var self = this;
  return new Promise(function(resolve, reject) {
    if (!fs.existsSync(path.join(self.project.path, 'config', '.org_connections'))) {
