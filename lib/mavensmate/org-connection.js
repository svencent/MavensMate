...
OrgConnectionService.prototype.add = function(username, password, orgType) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var storePromise = self.keychainService.useSystemKeychain() ? self.keychainService.storePassword(newConnectionId, password) : new Promise(function(resolve) { newConnection.password = password; resolve(); });

    storePromise
      .then(function(resolve, reject) {
        var orgConnectionClient = new SalesforceClient({ username: username, password: password, orgType: orgType });
        return orgConnectionClient.initialize()
      })
      .then(function() {
