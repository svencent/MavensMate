...
Renderer.prototype.render = function(command, renderToTmpFile) {
	...
	if (renderToTmpFile) {
		...
		swig.setDefaults({ safeMode: true, loader: swig.loaders.fs(path.join(__dirname,'templates')) }); // for some reason swig loses this context between express.js requests, need to set every time
		var rendered = swig.renderFile(self.template, self.context);
