...
function renderTree(treeOffset, packageLocation) {
	try {
    ...
    initAjax: {
    	...
    	url 	 : baseLocalServerURL+"/app/project/get-index",
	    data 	 : JSON.stringify({
...
function renderBufferedTree(metadata) {
	...
	tree = $("#tree").dynatree({
		...
		onLazyRead: function(node) {
			$.ajax({
	            url: baseLocalServerURL+"/execute/async",
	            data: {
...
function getPackage() {
    var json = { };
    var child_def = {};
    for (var item in child_metadata) {
