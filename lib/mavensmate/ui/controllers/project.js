...
ProjectController.prototype.update = function(req, res) {
  ...
  this.client.executeCommandForProject(req.project, 'edit-project', req.body, function(err, commandResult) {
    if (err) {
      jobQueue.finish(jobId, err, null);  
    } else {
      jobQueue.finish(jobId, null, commandResult);  
...
ProjectController.prototype.updateCreds = function(req, res) {
  this.client.executeCommandForProject(req.project, 'update-creds', req.body, function(err, commandResult) {
    if (err) {
      res.send(err); 
    } else {
...
ProjectController.prototype.updateSubscription = function(req, res) {
  this.client.executeCommandForProject(req.project, 'update-subscription', req.body, function(err, commandResult) {
    if (err) {
      res.send(err); 
    } else {
      res.send(commandResult); 
...
ProjectController.prototype.indexMetadata = function(req, res) {
  ...
  this.client.executeCommandForProject(req.project, 'index-metadata', req.body, function(err, commandResult) {
    if (err) {
      jobQueue.finish(jobId, err, null);  
    } else {
      jobQueue.finish(jobId, null, commandResult);  
...
ProjectController.prototype.getIndex = function(req, res) {
  this.client.executeCommandForProject(req.project, 'get-metadata-index', function(err, commandResult) {
    if (err) {
      res.send(err); 
    } else {
      res.send(commandResult); 
