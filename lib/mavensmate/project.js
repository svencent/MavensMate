...
Project.prototype.clean = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self._parsePackageXml()
      ...
      .catch(function(err) {
        reject('Error cleaning project: '+err.message);
      })
      .done(); 
  });
};
...
Project.prototype.setLightningIndex = function(index) {
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      reject(new Error('Could not write lightning index file: '+e.message));
    }  
  }); 
};
...
Project.prototype.getLightningIndex = function() {
  ...
  return new Promise(function(resolve, reject) {
    try {
      resolve(fs.readJsonSync(path.join(self.path, 'config', '.lightning')));
    } catch(err) {
      ...
      self.indexLightning()
        .then(function() {
          ...
          return self.getLightningIndex();
        })
        .catch(function(err) {
          reject(new Error('Could not get lightning index: '+err.message));
        });
    }
  });
};
            ...
            self._parsePackageXml()
              .then(function(packageXml) {
                ...
                _.forOwn(packageXml, function(packageMembers, metadataTypeXmlName) {
                  var metadataType = metadataService.getTypeByXmlName(metadataTypeXmlName); //inFolder, childXmlNames
                  if (_.has(metadataType, 'parentXmlName')) {
                    var parentMetadataType = metadataService.getTypeByXmlName(metadataType.parentXmlName);
                  }
                  if (packageMembers === '*') {
    ...
    Promise.resolve(fileProperties).then(function (properties) {
      ...
      try {
        ...
        _.each(properties, function(fp) {
          var metadataType;
          if (fp.type) {
            metadataType = self.metadataService.getTypeByXmlName(fp.type);
          } else {
