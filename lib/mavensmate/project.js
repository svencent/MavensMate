...
Project.prototype.compileMetadata = function(metadata) {
  ...
  var lightningService = new LightningService(self.sfdcClient);
  ...
  if (shouldCompileWithToolingApi && canCompileWithToolingApi) {
    ...
    self.sfdcClient.compileWithToolingApi(metadataToCompile, self)
      .then(function(result) {
        deferred.resolve(result);
      })
      ['catch'](function(error) {
...
Project.prototype.updateSetting = function(key, value) {
  ...
  var settings;
  try {
    settings = fs.readJsonSync(path.join(this.path, 'config', '.settings'));  
  } catch(err) {
    deferred.reject(new Error('Could not read project .settings file: '+err.message));  
  }
  ...
  try {
    ...
    deferred.reject(new Error('Could not write project .settings file: '+err.message));  
  }
  ...
  try {
    if (self.settings && self.settings.password && pw) {
      self.updateSetting('password', pw)
        ...
        ['catch'](function(err) {
          deferred.reject(new Error('Could not update password setting: '+err.message));
        })
        .done(); 
    } else {
      if (this.keychainService.useSystemKeychain()) {
