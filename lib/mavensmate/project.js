  ...
  return new Promise(function(resolve, reject) {
    if (!self._isValid()) {
      ...
      self.packageXml.init()
        ...
        .then(function() {
          self.getLocalStore();
          return self._getProjectSpecificSettings();
        })
        .then(function() {
...
Project.prototype._getProjectSpecificSettings = function() {
  var self = this;
  return new Promise(function(resolve, reject) {
    try {
      if (fs.existsSync(path.join(self.path, 'config.json'))) {
        config.overrides( fs.readJsonSync( path.join(self.path, 'config.json') ) );
