...
Project.prototype.initialize = function(isNewProject, isExistingDirectory) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (!isNewProject) {
      ...
      var initPromise = isExistingDirectory ? self._initFromExistingDirectory() : self._initNew();
      initPromise
        .then(function() {
          self.initialized = true;
          resolve(self);
 ...
 * @return {Promise}
 */
Project.prototype._initFromExistingDirectory = function() {
  var self = this;
  return new Promise(function(resolve, reject) {
    ...
    self.sfdcClient.describe()
      ...
      .then(function() {
        pkg = new Package({ path: path.join(self.path, 'src', 'package.xml') });
        return pkg.init();
      })
      .then(function() {
  ...
  return new Promise(function(resolve, reject) {
    if (!self._isValid()) {
      ...
      self.packageXml.init()
        ...
        .catch(function(error) {
          if (error.message.indexOf('INVALID_LOGIN') >= 0 || error.message.indexOf('EXPIRED_PASSWORD') >= 0) {
            self.valid = false;
          }
          reject(error);
        })
...
Project.prototype.replaceLocalFiles = function(remotePath) {
  var self = this;
  return new Promise(function(resolve, reject) {
...
Project.prototype.clean = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.packageXml.init()
      .then(function() {
        return self.sfdcClient.retrieveUnpackaged(self.packageXml.subscription, true, self.path);
      })
      .then(function(res) {
        retrieveResult = res;
        return fs.removeAsync(path.join(self.path, 'src'));
      })
      .then(function() {
        if (fs.existsSync(path.join(self.path, 'unpackaged'))) {
          fs.renameSync(path.join(self.path, 'unpackaged'), path.join(self.path, 'src'));
...
Project.prototype.edit = function(pkg) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.sfdcClient.retrieveUnpackaged(pkg, true, self.path)
      .then(function(retrieveResult) {
        fileProperties = retrieveResult.fileProperties;
        return fs.removeAsync(path.join(self.path, 'src'));
      })
                    ...
                    if (packageMembers === '*') {
                      ...
                      _.each(packageMembers, function(member) {
                        if (metadataType.inFolder) {
                          ...
                          var indexedType = _.find(orgMetadata, { 'xmlName': metadataTypeXmlName });
                          if (indexedType) {
                            // logger.silly('indexed type is: ',indexedType);
...
Project.prototype._storePassword = function(pw, replace) {
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      if (self.settings && self.settings.password && pw && !self.keychainService.useSystemKeychain()) {
    ...
    try {
      ...
      if (self.settings.password) {
        ...
        if (self.keychainService.useSystemKeychain()) {
          ...
          reject(new Error('System keychain is not support and there is no "password" property in project config/.settings file.'));
        }
      }
    } catch(err) {
