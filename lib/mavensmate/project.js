...
Project.prototype._writeDebug = function() {
  ...
  var locals = { userIds: [self.sfdcClient.conn.userInfo.user_id] };
  self.renderFile('debug.json', locals, filePath)
    .then(function() {
      deferred.resolve(); 
    })
    ['catch'](function(err) {
      deferred.reject(new Error('Could not write debug file: '+err.message));  
    })
    .done();
...
Project.prototype.renderFile = function(template, locals, destination) {
  var deferred = Q.defer();
  var rendered = swig.renderFile(template, locals);

  fs.outputFileSync(destination, rendered, function(err) {
    if (err) {
      deferred.reject(err);  
    } else {
      deferred.resolve();
