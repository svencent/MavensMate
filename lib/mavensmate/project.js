      ...
      .then(function() {
        self.sfdcClient.on('sfdcclient-new-log', function(message) {
          if (message.sobject && message.sobject.Id) {
            var logService = new LogService(self);
            logService.downloadLog(message.sobject.Id)
              ['catch'](function(error) {
 ...
 * @return {Promise}
 */
Project.prototype.indexSymbols = function() {
  var deferred = Q.defer();
  var self = this;

  if (!fs.existsSync(path.join(self.path, 'config', '.overlays'))) {
    fs.mkdirpSync(path.join(self.path, 'config', '.overlays')); 
  }
...
Project.prototype._storePassword = function() {
  ...
  try {
    if (config.get('mm_use_keyring')) {
      util.storePassword(this.id, this.password);
    } else {
...
Project.prototype._getPassword = function() {
  ...
  try {
    var pw;
    if (config.get('mm_use_keyring')) {
      pw = util.getPassword(this.settings.id);
      if (!pw) {
        if (!self.settings.password) {
          deferred.reject(new Error('Could not retrieve password from keystore or project settings file'));
        } else {
      ...
      if (!self.settings.password) {
        deferred.reject(new Error('Could not retrieve password from project settings file'));
      } else {
        pw = self.settings.password;
      }
    }
    if (pw) {
      ...
      deferred.reject(new Error('Could not retrieve password'));
    }
  } catch(e) {
