 ...
 * Cached session
 */
Object.defineProperty(Project.prototype, 'session', {
  get: function() {
    ...
    if (!isNewProject) {  
      self._initExisting()
        ...
        .catch(function(error) {
          reject(new Error('Could not initiate existing Project instance: '+error));
        })
        .done(); 
    } 
      ...
      self._initNew()
        ...
        .catch(function(error) {
          reject(new Error('Could not initiate new Project instance: '+error));
        })
        .done();
    }
  });
    ...
    if (fs.existsSync(self.path)) {
      ...
      self.sfdcClient.describe()
        ...
        .catch(function(error) {
          ...
          reject(new Error('Could not retrieve and write project to file system: '+error.message));
        })
        .done();
    } 
  });
...
Project.prototype._initConfig = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Promise.all(promises)
      ...
      .catch(function(error) {
        reject(new Error('Could not initiate project config directory: '+error.message));
      })
      .done();
  }); 
};
...
Project.prototype._parsePackageXml = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    fs.readFile(path.join(self.path, 'src', 'package.xml'), function(err, data) {
      if (err) {
        reject(err);
      } else {
        var sax = require('sax'),
...
Project.prototype.refreshFromServer = function(metadata) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var unpackagedPath = path.join(self.workspace, self.name, 'unpackaged');
      ...
      .then(function() {
        ...
        finder.on('file', function (file) { 
          ...
          if (fileBasename !== 'package.xml') {
            ...
            var destinationDirectory = directory.replace(path.join(self.workspace, self.name, 'unpackaged'), path.join(self.workspace, self.name, 'src')); //=> /foo/bar/myproject/src/classes
...
Project.prototype._getClientProjectSettings = function() {
  var self = this;
  return new Promise(function(resolve, reject) {
    if (fs.existsSync(path.join(self.path, self.name+'.json'))) {
      config.file('project', path.join(self.path, self.name+'.json'));
      resolve('Project settings loaded.');
    } else {
      resolve('No user project settings.');
    }
  });
};
...
Project.prototype.updateSetting = function(key, value) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      reject(new Error('Could not write project .settings file: '+err.message));  
    }
  }); 
};
...
Project.prototype.updateCreds = function(creds) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.sfdcClient.initialize()
      ...
      .catch(function(err) {
        reject(new Error('Could not update credentials: '+err.message));
      })
      .done();
  }); 
};
    ...
    fs.readJson(path.join(self.path, 'config', '.settings'), function(err, settings) {
      if (err) {
        ...
        self._getPassword()
          ...
          .catch(function(err) {
            reject(new Error('Could not get project settings: '+err));
          })
          .done();
      }
    });
...
// retrieves debug from config/.debug
Project.prototype._getDebug = function() {
  var self = this;
  return new Promise(function(resolve, reject) {
    fs.readJson(path.join(self.path, 'config', '.debug'), function(err, debug) {
      if (err) {
        reject(err);
      } else {
        self.debug = debug;
...
Project.prototype.setLightningIndex = function(index) {
  var self = this;
  return new Promise(function(resolve, reject) {
    fs.outputFile(path.join(self.path, 'config', '.lightning'), JSON.stringify(index, null, 4), function(err) {
      if (err) {
        throw new Error('Could not write lightning index file');
      } else {
...
Project.prototype.getLightningIndex = function() {
  try {
    return fs.readJsonSync(path.join(this.path, 'config', '.lightning'));
  } catch(err) {
    throw new Error('Could not get lightning index');
  }
};
...
Project.prototype.indexLightning = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    lightningService.getAll()
      ...
      .catch(function(err) {
        if (err.message.indexOf('sObject type \'AuraDefinition\' is not supported') >= 0) {
          resolve();
        } else {
          reject(new Error('Could not index lightning: '+err.message));        
        }
      })
      .done();
  }); 
...
Project.prototype.indexSymbols = function(apexClassName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    symbolPromise
      ...
      .catch(function(err) {
        reject('Could not index org metadata: '+err.message);
      })
      .done();
  }); 
};
        ...
        fs.readJson(path.join(self.path, 'config', '.org_metadata'), function(err, orgMetadata) {
          if (err) {
            ...
            self._parsePackageXml()
              ...
              .catch(function(err) {
                reject(new Error('Can not obtain org metadata index: '+err.message));
              })
              .done();
          }
        });
    ...
    Promise.resolve(fileProperties).then(function (properties) {
      ...
      try {
        ...
        _.each(properties, function(fp) {
          var metadataType;
          if (fp.type) {
            metadataType = self.metadataService.getTypeByName(fp.type);
          } else {
            metadataType = self.metadataService.getTypeByPath(fp.fileName.split('.')[1]);
          }
        ...
        reject(new Error('Could not update local store: '+e.message));
      }  
    });
  });
};
    ...
    Promise.resolve(fileProperties)
      .then(function (properties) {
        try {
          ...
          reject(new Error('Could not initiate local store: '+e.message));  
        }
      });
  });
};
    ...
    try {
      if (self.settings && self.settings.password && pw) {
        self.updateSetting('password', pw)
          ...
          .catch(function(err) {
            reject(new Error('Could not update password setting: '+err.message));
          })
          .done(); 
      } else {
        if (self.keychainService.useSystemKeychain()) {
      ...
      reject(new Error('Could not store password: '+e.message));
    }
  });
};
...
Project.prototype._getPassword = function() {
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      reject(new Error('Could not retrieve password: '+e.message));
    }
  });
};
