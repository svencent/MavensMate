  ...
  return new Promise(function(resolve, reject) {
    if (!self._isValid()) {
      ...
      self.packageXml.init()
        ...
        .catch(function(error) {
          if (error.message.indexOf('INVALID_LOGIN') >= 0) {
            self.valid = false;
          }
          reject(error);
        })
...
Project.prototype.getOrgMetadataIndexWithSelections = function(keyword, ids) {
  var self = this;
  return new Promise(function(resolve, reject) {
                ...
                if (packageMembers === '*') {
                  ...
                  _.each(packageMembers, function(member) {
                    if (metadataType.inFolder) {
                      ...
                      var indexedType = _.find(orgMetadata, { 'xmlName': metadataTypeXmlName });
                      if (indexedType) {
                        logger.debug('indexed type is: ',indexedType);
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      if (self.settings.password) {
        ...
        if (self.keychainService.useSystemKeychain()) {
          self.keychainService.getPassword(self.settings.id)
