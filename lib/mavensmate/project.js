    ...
    if (!self._isValid()) {
      ...
      self.packageXml.init()
        ...
        .then(function(tokens) {
          if (!self.sfdcClient) {
            logger.debug('creating new sfdc client', self.settings, tokens);
            self.sfdcClient = new SalesforceClient({
              ...
              orgType: self.settings.environment
            });
            self.sfdcClient.on('token-refresh', function() {
              logger.debug('project caught event: token-refresh');
              self._storeTokens(true)
                .catch(function(err) {
                  logger.error('project could not store refreshed tokens');
                  throw new Error('Could not update project tokens:', err);
                });
            });
          }
...
Project.prototype.updateCreds = function(creds) {
  ...
  return new Promise(function(resolve, reject) {
    var username = creds.username || self.sfdcClient.username;
    var accessToken = creds.accessToken || self.sfdcClient.accessToken;
    var refreshToken = creds.refreshToken || self.sfdcClient.refreshToken;
    var orgType = creds.orgType || self.sfdcClient.orgType;
    var loginUrl = creds.loginUrl || self.sfdcClient.loginUrl;
    var instanceUrl = creds.instanceUrl || self.sfdcClient.instanceUrl;
    self.sfdcClient = new SalesforceClient({
      username: username,
    ...
    self.sfdcClient.initialize()
      .then(function() {
        return self._storeTokens(true);
      })
      .then(function() {
        return self.updateSetting('username', username);
      })
      .then(function() {
        return self.updateSetting('environment', orgType);
      })
      .then(function() {
        return self.updateSetting('loginUrl', loginUrl);
      })
      .then(function() {
        return self.updateSetting('instanceUrl', instanceUrl);
      })
      .then(function() {
      ...
      .then(function() {
        if (!self.valid) {
          logger.debug('project was not valid, running init again');
...
Project.prototype._writeSettings = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    // if keychain service is not enabled, we need to write tokens in plain text (USER SHOULD ENABLE KEYCHAIN IF AT ALL POSSIBLE!)
    if (!self.keychainService.useSystemKeychain()) {
      settings.accessToken = self.sfdcClient.accessToken;
      settings.refreshToken = self.sfdcClient.refreshToken;
    }
...
/**
 * Stores accessToken/refreshToken for project
 * @param  {[type]} pw      [description]
 * @param  {[type]} replace [description]
 * @return {[type]}         [description]
 */
Project.prototype._storeTokens = function(replace) {
  var self = this;
  return new Promise(function(resolve, reject) {
    try {
      logger.debug('_storeTokens');
      if (self.keychainService.useSystemKeychain()) {
        logger.debug('storing tokens in system keychain');
        if (replace) {
          self.keychainService.replacePassword(self.id || self.settings.id, self.sfdcClient.accessToken, 'accessToken');
          self.keychainService.replacePassword(self.id || self.settings.id, self.sfdcClient.refreshToken, 'refreshToken');
        } else {
          self.keychainService.storePassword(self.id || self.settings.id, self.sfdcClient.accessToken, 'accessToken');
          self.keychainService.storePassword(self.id || self.settings.id, self.sfdcClient.refreshToken, 'refreshToken');
        }
        // clear out any passwords that were previously stored in .settings
        self.updateSetting('accessToken', undefined)
          .then(function() {
            return self.updateSetting('refreshToken', undefined);
          })
          .then(function() {
            resolve();
          })
          .catch(function(err) {
            logger.error('Could not clear tokens from settings -->'+err.message);
            reject(err);
          })
          .done();
      } else {
        logger.debug('storing tokens in .settings');
        self.updateSetting('accessToken', self.sfdcClient.accessToken)
          .then(function() {
            return self.updateSetting('refreshToken', self.sfdcClient.refreshToken);
          })
          .then(function() {
            resolve();
          })
          .catch(function(err) {
            logger.error('Could not update tokens in settings -->'+err.message);
            reject(err);
          })
          .done();
      }
    } catch(err) {
      logger.error('Could not store tokens -->'+err.message);
      reject(err);
    }
  });
};
...
/**
 * Retrieves access/refresh tokens from settings or the keychain
 * @return {Promise}
 */
Project.prototype._getTokens = function() {
  var self = this;
  return new Promise(function(resolve, reject) {
    try {
      // if the tokens are specified in config/.settings, it overrides their desire/ability to use keychain
      if (self.settings.accessToken && self.settings.refreshToken) {
        resolve({
          accessToken: self.settings.accessToken,
          refreshToken: self.settings.refreshToken
        });
      } else {
        if (self.keychainService.useSystemKeychain()) {
          try {
            resolve({
              accessToken: self.keychainService.getPassword(self.settings.id, 'accessToken'),
              refreshToken: self.keychainService.getPassword(self.settings.id, 'refreshToken')
            });
          } catch(e) {
            logger.error(e);
            reject(new Error('Could not retrieve project access/refresh tokens from the system keychain. If you do not wish to use the system keychain, set "mm_use_keyring" to false, then specify valid access/refresh tokens in your project\'s config/.settings file.'));
          }
        } else {
          reject(new Error('System keychain is not enabled and access/refresh tokens could not be found in your project\'s config/.settings file.'));
        }
      }
    } catch(err) {
      logger.error('Could not retrieve tokens -->'+err.message);
      reject(err);
    }
  });
};
