...
Command.prototype.execute = function() {
  ...
  if (self.isUICommand() && self.client.editor === 'sublime') {
    ...
    editorService.launchUI('new-lightning-interface', { pid: self.getProject().settings.id });
    ...
    return lightningService.createBundle(apiName, self.payload.description)
      ...
      .then(function() {
        self.respond('Lightning interface created successfully');
      })
      .catch(function(error) {
        self.respond('Could not create lighting interface', false, error);
      })
      .done(); 
  } 
};
