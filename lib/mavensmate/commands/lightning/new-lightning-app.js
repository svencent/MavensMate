...
Command.prototype.execute = function() {
  ...
  return new Promise(function(resolve, reject) {
    if (self.isUICommand() && self.client.editor === 'sublime') {
      ...
      editorService.launchUI('new-lightning-app', { pid: self.getProject().settings.id })
        .then(function() {
          resolve('Success');
        })
        .catch(function(error) {
          reject({
            message: 'Could not open new lightning app UI',
            error: error
          })
        });
    } else {
      ...
      return lightningService.createBundle(apiName, self.payload.description)
        ...
        .catch(function(error) {
          reject({
            message: 'Could not create lighting application',
            error: error
          });
        })
        .done(); 
