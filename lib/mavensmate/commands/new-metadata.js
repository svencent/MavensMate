	...
	return deploy.execute(newMetadata)
		.then(function(result) {
			if (self.client.editor) {
				var path = path.join(project.path, 'src', newMetadata.getType().directoryName, [newMetadata.getName(), newMetadata.getType().suffix].join('.'));
				new EditorService(self.client).open(path);
			} 
			self.respond(result);
		})
