/* session commander component
 * To use add require('../cmds/session.js')(program) to your commander.js based node executable before program.parse
 */
'use strict';

var util 									= require('../util').instance;
var inherits 							= require('inherits');
var BaseCommand 					= require('../command');
var SalesforceClient 			= require('../sfdc-client');

function Command() {
	Command.super_.call(this, Array.prototype.slice.call(arguments, 0));
}

inherits(Command, BaseCommand);

Command.prototype.execute = function() {
	var self = this;

	var sfdcClient = new SalesforceClient(self.payload);
	sfdcClient.initialize()
		.then(function() {
			return sfdcClient.describe();
		})
		.then(function(describe) {
			console.log(sfdcClient);
			var response = {
				sid: sfdcClient.conn.accessToken,
				metadata_server_url: sfdcClient.conn.serverUrl,
				server_url: sfdcClient.conn.serverUrl,
				org_metadata_types: describe.metadataTypes
			};
			self.respond(response);
		})
		['catch'](function(error) {
			self.respond('Could not create new Salesforce session', false, error);
		})
		.done();
};

exports.command = Command;
exports.addSubCommand = function(client) {
	client.program
		.command('session [username] [password] [org_type]')
		.alias('get-active-session')
		.version('0.0.1')
		.option('-u <username>', 'salesforce.com username')
		.option('-p <password>', 'salesforce.com password')
		.option('-o <org_type>', 'Type of org: prod, dev, sandbox, custom')
		.option('--verbose', 'Returns server urls and describe information')
		.description('Creates new salesforce.com session, returns session id')
		.action(function(username, password, orgType) {
			if (username && password && orgType) {
				var payload = {
					username : username,
					password: password,
					orgType: orgType
				};
				client.executeCommand(this._name, payload);	
			} else {
				util.getPayload()
					.then(function(payload) {
						client.executeCommand(this._name, payload);	
					});	
			}
		});
};