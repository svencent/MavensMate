...
Object.defineProperty(MavensMateFile.prototype, 'localMembers', {
  get: function() {
    ...
    _.each(directoryFiles, function(f) {
      console.log('->');
      console.log(path.join(self.path, f));
      if (!_.find(contents, { path: path.join(self.path, f)  })) {
      ...
      if (!path.extname(f)) {
        ...
        _.each(subDirectoryFiles, function(sf) {  
          console.log('----->');
          console.log(path.join(self.path, f, sf));
          if (!_.find(contents, { path: path.join(self.path, f, sf)  })) {
...
module.exports.getToolingFiles = function(files) {
  return _.filter(files, function(f) { return f.isToolingType; });
};
