...
Metadata = function(opts) {
  ...
  if (this.sfdcClient || this.project) {
    this.metadataHelper = new MetadataHelper({ sfdcClient : this.sfdcClient || this.project.sfdcClient });
  } else {
    this.metadataHelper = new MetadataHelper();
  }
  
  if (this.metadataTypeXmlName) {
    this.type = this.metadataHelper.getTypeByXmlName(this.metadataTypeXmlName); 
    if (this.template) {
 ...
 */
MetadataHelper.prototype.getMetadataFromPaths = function(paths, project) {
  var self = this;
  return new Promise(function(resolve, reject) {
    var metadata = [];
