...
Metadata = function(opts) {
  ...
  if (this.template && this.metadataTypeXmlName) {
...
MetadataService.prototype.getTypeByPath = function(p) {
  ...
  var grandparentPath = path.dirname(path.dirname(p));
  var existsOnFileSystem = fs.existsSync(p);
  if (existsOnFileSystem && fs.lstatSync(p).isDirectory() && path.basename(parentPath) === 'aura') {
    return this.getTypeByDirectoryName('aura');
  } else if (existsOnFileSystem && path.basename(grandparentPath) === 'aura') {
    return this.getTypeByDirectoryName('aura');
  } else {    
    var folderBasedTypes = _.where(this.parentTypes, { 'inFolder': true });
...
MetadataService.prototype.getMetadataFromPaths = function(paths, project) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    project.packageService.deserialize()
      .then(function(ppkg) {
        projectPackage = ppkg;
        return project.getOrgMetadataIndex();
      })
      .then(function(pindex) {
        ...
        _.each(paths, function(p) {
          if (!path.extname(p)) { // this is a directory, need to create metadata instances for all package members for that type
            ...
            var typePackageSubscription = projectPackage[metadataType.xmlName];
            ...
            if (typePackageSubscription === '*') {
              ...
              if (metadataType.inFolder) {
                ...
                _.each(typePackageSubscription, function(sub) {
                  console.log(sub);
                  console.log(path.join(project.path, 'src', metadataType.directoryName, sub));
                  console.log(path.join(project.path, 'src', metadataType.directoryName, [sub, '-meta.xml'].join('.')));
                  if (path.extname(sub)) {
