...
LightningService.prototype.deleteBundleItems = function(sfdcPaths) {
  var self = this;
  return new Promise(function(resolve, reject) {
    if (sfdcPaths.length === 0) {
      return resolve();
    } else {
      Promise.resolve(self.project.getLightningIndex())
        .then(function(lightningIndex) {
          logger.debug(lightningIndex);
          var deleteIds = [];
          _.each(sfdcPaths, function(sp) {
            var lightningBundleName = sp.folderName; //mybundle
            var lightningType = sp.lightningType;
...
LightningService.prototype.getBundleItems = function(sfdcPaths) {
  var self = this;
  return new Promise(function(resolve, reject) {
    if (sfdcPaths.length === 0) {
      return resolve();
 ...
 * @return {Promise}          
 */
LightningService.prototype.update = function(metadata) {
  var self = this;
  return new Promise(function(resolve, reject) {
      ...
      Promise.resolve(self.project.getLightningIndex())
        .then(function(lightningIndex) {
          ...
          _.each(metadata, function(m) {
            var lightningBundleName = path.basename(path.dirname(m.path)); //mybundle
            var lightningType = m.getLightningDefinitionType();
