...
SalesforceClient.prototype.getUsername = function() {
  if (this.conn !== undefined && this.conn.userInfo !== undefined) {
...
SalesforceClient.prototype.initialize = function() {
  ...
  if (accessToken !== undefined && accessToken !== '') {
    // global.logger.debug('attempting to use cached session information');

    self.conn = new jsforce.Connection({ sessionId: accessToken, instanceUrl: self.getInstanceUrl(), logLevel: 'FATAL', version: global.mmApiVersion });
      ...
      self.conn.tooling.query('Select ParentJobId FROM ApexTestQueueItem WHERE Id = \''+testQueueItemId+'\'', function(err, res) {
        if (err) { 
          ...
          var poll = function() {
            ...
            self._checkTest(parentJobId).then(function(results) {
              global.logger.debug('test status: ');
              global.logger.debug(results);
              if (results.size === numberOfTests) {
