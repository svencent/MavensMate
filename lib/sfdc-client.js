...
SalesforceClient.prototype.retrieveUnpackaged = function(metadataTypesOrPackage) {
  ...
  self.conn.metadata.retrieve({ 
    ...
    .complete(function(err, result) {
      ...
      console.log(result);
      if (err) {
        deferred.reject(new Error('Could not retrieve metadata from salesforce.com: '+err.message));
      } else {
        var rstream = new Stream();
        rstream.readable = true;
        rstream.emit('data', new Buffer(result.zipFile, 'base64'));
        rstream.emit('end');
        deferred.resolve({ stream:rstream, fileProperties:result.fileProperties });
      }
    });
