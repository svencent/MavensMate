 ...
 */
SalesforceClient.prototype.toolingCompile = function(files) {
  var deferred = Q.defer();
  var self = this;
...
SalesforceClient.prototype._createMember = function(file, containerId) {
  ...
  var metadataType = util.getMetadataBySuffix(ext);
  var memberName = metadataType.xmlName+'Member';

  self.conn.tooling.sobject(memberName).create({
    Body: util.getFileBody(file),
    MetadataContainerId: containerId,
    ContentEntityId: global.project.localStore[basename].id
  }, function(err, res) {
    if (err) { 
      deferred.reject(err);
    } else {
