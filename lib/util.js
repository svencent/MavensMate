...
MavensMateUtil.prototype.getMetadataByName = function(name) {
  var type;
...
MavensMateUtil.prototype.getMetadataBySuffix = function(suffix) {
  var type;
  _.each(global.describe.metadataObjects, function(metadataType) {
...
MavensMateUtil.prototype.getMetadataByDirectoryName = function(dirName) {
  var type;
  _.each(global.describe.metadataObjects, function(metadataType) {
...
MavensMateUtil.prototype.putKey = function(seed) {
  var algorithm = 'aes256'; // or any other algorithm supported by OpenSSL
  var key = 'password';

  var cipher = crypto.createCipher(algorithm, key);  
  var encrypted = cipher.update(seed, 'utf8', 'hex') + cipher.final('hex');
  // store somewhere
};

MavensMateUtil.prototype.getKey = function() {
  var algorithm = 'aes256'; // or any other algorithm supported by OpenSSL
  var key = 'password';
...
// extracts a zip file from the given path to the given destination
MavensMateUtil.prototype.extractZipFile = function(zipPath, destination) {
  var deferred = Q.defer();

  if (destination === undefined) {
  	destination = path.dirname(zipPath);
  }
