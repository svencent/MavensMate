...
MavensMateUtil.prototype.extractZipFile = function(zipPath, destination) {
  ...
  var gzip = readStream.pipe(zlib.createGzip());
  gzip.pipe(fs.createWriteStream(destination))
  ...
  // var writeStream = fstream.Writer(destination);
  ...
  //     });
	...
	//   .pipe(unzip.Extract({ path: 'Users/josephferraro/Desktop' }))
	//     .on('error', function(err) {
	//       console.log('error extracting -->');
 //        console.log(err);
 //        deferred.reject(err);
