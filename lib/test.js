...
ApexTest.prototype._initialize = function() {
  ...
  _.each(this.tests, function(t) {
    if (t.indexOf(path.sep) !== -1) {
      if (t.indexOf(global.project.path) === -1) {
        throw new Error('Referenced file is not a part of this project: '+t);
      }
    } else {
      var fullPath = path.join(global.project.path, 'src', 'classes', t);
      console.log(fullPath);
      if (fullPath.indexOf(global.project.path) === -1 || !path.existsSync(fullPath)) {
        throw new Error('Referenced file is not a part of this project: '+t);
      }
    }
...
ApexTest.prototype.execute = function() {
  ...
  var localStore = global.project.localStore;
  var tests = [];
  var testClassIds = [];
  ...
  global.sfdcClient.runTests(tests)
    .then(function(results) {
      testResults = results;

      var projectClassIds = [];
      _.forOwn(global.project.localStore, function(value, key) {
        if (key.endsWith('.cls')) {
...
ApexTest.prototype.getCoverage = function(classIds, testClassIds) {
  ...
  global.sfdcClient.conn.tooling.query(query, function(err, res) {
    if (err) { 
      deferred.reject(err);
    } else {
