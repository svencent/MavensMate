  ...
  if (fs.existsSync(self.path)) {
    ...
    global.sfdcClient.retrieveUnpackaged(self.package)
      .then(function(retrieveResult) {
        ...
        retrieveFileProperties = retrieveResult.fileProperties;
      ...
      .then(function() {
        // return self._initLocalStore(retrieveFileProperties);
      })
      .then(function() {
        deferred.resolve();
      })
...
Project.prototype._initLocalStore = function(fileProperties) {
  ...
  var filePath = path.join(this.path, 'config', '.local_store');
