 ...
 * @return {Promise}
 */
Metadata.getServerProperties = function(metadataTypes) {
  var deferred = Q.defer();
  ...
  _.each(metadataTypes, function(type) {
    ...
    //if (typeMap[type].)
    listRequests.push(global.sfdcClient.list(type));
  });
  ...
  Q.all(listRequests)
    .then(function(results) {      
      ...
      _.each(results, function(metadataListResult) {
        _.forOwn(metadataListResult, function(items, key) {
          var metadataTypeDef = typeMap[key];
          response[key] = metadataTypeDef;
          // response[key].type = metadataTypeDef; todo: causing circular ref error
          response[key].id = key;
          response[key].title = key;
          response[key].key = key;
          response[key].level = 1; //todo
          response[key].hasChildTypes = _.has(typeMap[key], 'childXmlNames');
          response[key].isFolder = false; //todo
          response[key].cls = 'folder'; //todo
          response[key].select = true; //todo
          response[key].expanded = false; //todo

          _.each(items, function(item) {
            item.leaf = true;
            item.title = item.fullName;
            item.checked = false; //todo
            item.id = [key, item.fullName].join('.');
            item.text = item.fullName;
            item.cls = ''; //todo
            item.level = 2; //todo
            item.isFolder = false; //todo
            item.children = []; //todo
          });

          response[key].children = items;
