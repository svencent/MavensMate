...
Metadata.prototype._writeFile = function(deployPath) {
  ...
  self._getTemplateBody()
    ...
    ['catch'](function(err) {
      deferred.reject(new Error('Could not write metadata file based on template: '+JSON.stringify(err)));
    })
    .done();
  return deferred.promise;
};
    ...
    if (err) { 
      ...
      Metadata._writePackage(metadataObject, path.join(tmpPath, 'unpackaged'))
        ...
        ['catch'](function(error) {
          ...
          deferred.reject(new Error('Could not deploy metadata to server: '+JSON.stringify(error)));
        })
        .done();           
    }
  });
  ...
  tmp.dir({ prefix: 'mm_' }, function _tempDirCreated(err, newPath) {
    if (err) { 
      ...
      Metadata._writePackage(metadataObject, path.join(newPath, 'unpackaged'), 'destructiveChanges.xml')
        ...
        ['catch'](function(err) {
          deferred.reject(new Error('Could not prepare metadata for deletion: '+JSON.stringify(err)));
        })
        .done();
    }
  });
  ...
  tmp.dir({ prefix: 'mm_' }, function _tempDirCreated(err, newPath) {
    if (err) { 
      ...
      Metadata._writePackage(metadataObject, path.join(newPath, 'unpackaged'))
        ...
        ['catch'](function(err) {
          deferred.reject(new Error('Could not prepare metadata for deployment: '+JSON.stringify(err)));
        })
        .done();
    }
  });
