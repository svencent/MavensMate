...
/**
 * Looks in config and the path to find supported editors (sublime, atom, code, etc.)
 * @return {Nothing}
 */
Client.prototype._detectEditors = function() {
 ...
 */
Client.prototype.addProjectByPath = function(projectPath) {
  var self = this;
...
/**
 * Adds a project to the client by passing the project id
 * @param {String} projectId  - id of the project (found in config/.settings)
 */
Client.prototype.addProjectById = function(projectId) {
  var self = this;
  return new Promise(function(resolve, reject) {
...
/**
 * Watches mavensmate config for updates, when it detects an update, reloads local config
...
Client.prototype._runFinishCommands = function(project, command, body, editor) {
  ...
  var self = this;
  if (self.finishCommands[command]) {
    var finishCommandsForCommand = self.finishCommands[command];
    var finishPromises = [];
...
Client.prototype.executeCommand = function(payload) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    } else if (!project && self.getProjects().length > 0) {
...
_.each(commandFiles, function(filepath) {
  Client[capitalize(camelize(path.basename(filepath).split('.')[0])+'Command')] = require(filepath).command;
});
